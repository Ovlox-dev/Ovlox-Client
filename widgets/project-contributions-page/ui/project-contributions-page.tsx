        ...
        <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
            <header className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold mb-1 flex items-center gap-2">
                        <Users className="size-6" /> Contributions
                    </h1>
            ...
            {isPending ? (
                ...
                <Card className="p-12 flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm text-muted-foreground">No contributions in this range.</p>
                </Card>
            ) : (
                <div className="space-y-2">
                    {contributors.map((c, idx) => {
                        ...
                        return (
                            <Card key={c.key} className="p-3 flex items-center gap-3 flex-wrap sm:flex-nowrap">
                                <span className="text-xs font-mono text-muted-foreground w-6 shrink-0">#{idx + 1}</span>
                                <Avatar className="size-9 shrink-0">
                                    <AvatarFallback>{initials}</AvatarFallback>
                                </Avatar>
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium truncate">{c.name || c.email || "Unknown"}</p>
                                    {c.email && c.name && (
